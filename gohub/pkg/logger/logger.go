...
func InitLogger(fileName string, maxSize, maxBackup, maxAge int,
	...
	Logger = zap.New(core,
		...
		zap.AddStacktrace(zap.ErrorLevel)) // Error 时才会显示 stacktrace 堆栈
...
func getEncoder() zapcore.Encoder {
	encoderConfig := zapcore.EncoderConfig{
		...
		// 如：types/converter.go:17，长格式为绝对路径
	}
...
func Dump(value any, mes ...string) {
	valueString := jsonString(value)
	// 判断第二个参数是否传参 msg
	if len(mes) > 0 {
		Logger.Warn("Dump", zap.String(mes[0], valueString))
	} else {
