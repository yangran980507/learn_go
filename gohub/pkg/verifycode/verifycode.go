...
func (vc *VerifyCode) SendSMS(phone string) bool {
	...
	if app.IsProduction() && strings.HasPrefix(phone,
		config.GetString("verifycode.debug_prefix")) {
		return true
	}
...
func (vc *VerifyCode) SendEmail(email string) error {
	...
	//方便本地和 API 测试
	if !app.IsProduction() && strings.HasSuffix(email,
		config.GetString("verifycode.debug_email_suffix")) {
		return nil
	}
...
func (vc *VerifyCode) CheckAnswer(key string, answer string) bool {
	...
	if !app.IsProduction() &&
		(strings.HasSuffix(key, config.GetString("verifycode.debuug_email_suffix")) ||
...
func (vc *VerifyCode) generateVerifyCode(key string) string {
	...
	//将验证码放置及 KEY （邮箱或手机号）存放到 Redis 中并设置过期时间
