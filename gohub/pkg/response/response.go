...
// 执行某个 『没有具体返回数据』 的 『变更』 操作后调用，例如删除、修改密码、修改手机号
...
func Abort404(c *gin.Context, msg ...string) {
	c.AbortWithStatusJSON(http.StatusNotFound, gin.H{
		"message": defaultMessage("数据不存在，请确定请求方式", msg...),
	})
}

// About403 响应 403，未传参 msg 时使用默认消息
func About403(c *gin.Context, msg ...string) {
	c.AbortWithStatusJSON(http.StatusForbidden, gin.H{
		"message": defaultMessage("权限不足，清确定您有对应的权限", msg...),
	})
}
...
// 在解析用户请求，请求的格式或者方法阿不符合预期时候调用
func BadRequest(c *gin.Context, err error, msg ...string) {
	logger.LogIf(err)
	c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{
		"message": defaultMessage("请求解析错误，请确认请求格式是否正确。上传文件请使用"+
			"multipart 标头，参数请使用 JSON 格式。", msg...),
		"error": err.Error(),
	})
}
...
// Unauthorized 响应 401,传参 err 对象，未传参 msg 时使用默认消息
// 登陆失败，jwt 解析失败时调用
